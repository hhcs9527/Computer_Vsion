...
def nms_compare(Harris_Response, x_d, y_d, angle, gradient, i, j):
    #q ,r = 0, 0
    # angel 0
    ...
    if not((gradient[i, j] >= q) and (gradient[i, j] >= r)):
        return Harris_Response[i, j]

    return 0


def non_max_subpression(x_d, y_d, Harris_Response):
    ...
    result = np.zeros(Harris_Response.shape)

    for i in range(1, l0-1):
        for j in range(1, l1-1):
            result[i, j] = nms_compare(Harris_Response, x_d, y_d, angle, gradient, i, j)
...
def Harris_Response(x_d, y_d, k, window_size):
    ...
    Harris_Response  = non_max_subpression(x_d, y_d, Harris_Response)    
...
def type_distinguish(Response, img):
    ...
    img_result[Response > 0.001*Response.max()] = 255
    return img_result
